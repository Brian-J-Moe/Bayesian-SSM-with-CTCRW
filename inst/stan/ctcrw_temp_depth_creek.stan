 ...
 * logistic predation refuge attraction force as a function of length
 *=============================================================================*/
...
functions {
  ...
  real filter_segment_1d(int idx_start, int n_obs, 
    ...
    for (i in 2:n_obs) {
      ...
      real D_pref = d_inf_z - (d_inf_z - d_0) * exp(-kappa_d * age_t[idx]);
      real f_depth = omega * (D_pref - depth_z[idx]) * depth_grad_z[idx];
      
      real w_creek = 1.0 / (1.0 + exp(kappa_c * (length_t[idx] - length_50)));
      real f_creek = psi * w_creek * creek_grad_z[idx];
      
      real f = f_temp + f_depth + f_creek;
  ...
  real partial_sum_lpmf(array[] int seg_slice, int start, int end,
                        array[] int seg_start, array[] int seg_length, array[] int seg_id, 
                        array[] real age_t, array[] real length_t,
                        ...
                        vector creek_dX_z, vector creek_dY_z,
                        real T_opt_z, real alpha, real d_inf_z, 
                        real d_0, real kappa_d, real omega, 
                        real kappa_c, real psi, real length_50) {
    real ll = 0;
    ...
    for (k in 1:size(seg_slice)) {
      ...
      // Filter 
      ll += filter_segment_1d(idx_s, n_s, age_t, length_t, 
                              obs_x, dt_vec, hpe_vec,
                              beta[ind], sigma[ind], 
                              temp_z, temp_dX_z,
                              depth_z, depth_dX_z,
                              creek_dX_z,
                              ...
                              d_0, kappa_d, omega,
                              kappa_c, psi, length_50);
                              
      ll += filter_segment_1d(idx_s, n_s, age_t, length_t, 
                              obs_y, dt_vec, hpe_vec,
                              beta[ind], sigma[ind], 
                              temp_z, temp_dY_z,
                              depth_z, depth_dY_z,
                              creek_dY_z,
...
data {
  ...
  // Position based creek gradients
  vector[N_total] creek_dX;
  vector[N_total] creek_dY;
...
transformed data {
  ...
  // z-score creek gradients
  
  vector[2 * N_total] combined_creek_grad;
  combined_creek_grad = append_row(creek_dX, creek_dY);
  
  real creek_grad_sd = sd(combined_creek_grad);
  vector[N_total] creek_dX_z = creek_dX / creek_grad_sd;
  vector[N_total] creek_dY_z = creek_dY / creek_grad_sd;
...
transformed parameters {
  ...
  real<lower=0> kappa_c = exp(log_kappa_c);
...
model {
  ...
  // Creek transition parameters
  length_50 ~ normal(length_trans, 20);
  log_kappa_c ~ normal(log(0.05), 0.5);
  log_psi ~ normal(0, 1);
  ...
  target += reduce_sum(partial_sum_lpmf, 
                       ...
                       seg_id, age_t, length_t, 
                       obs_x, obs_y, dt, HPEm, beta, sigma, 
                       temp_z, temp_dX_z, temp_dY_z,
                       depth_z, depth_dX_z, depth_dY_z,
                       creek_dX_z, creek_dY_z,
                       T_opt_z, alpha, d_inf_z, 
                       d_0, kappa_d, omega,
                       kappa_c, psi, length_50);
}
