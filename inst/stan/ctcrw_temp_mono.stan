/*=============================================================================
 * Bayesian CTCRW State-Space Model with monotonic temperature drift
 *=============================================================================*/
...
functions {
  ...
  real filter_segment_1d(int idx_start, int n_obs, 
                         ...
                         vector temp_z, vector grad_z,
                         real temp_max_z, real alpha) {
    real ll = 0;
    ...
    for (i in 2:n_obs) {
      ...
      real f = alpha * (temp_max_z - temp_z[idx]) * grad_z[idx];
  ...
  real partial_sum_lpmf(array[] int seg_slice, int start, int end,
                        ...
                        vector temp_z, vector dX_z, vector dY_z,
                        real temp_max_z, real alpha) {
    real ll = 0;
    ...
    for (k in 1:size(seg_slice)) {
      ...
      ll += filter_segment_1d(idx_s, n_s, obs_x, dt_vec, hpe_vec,
                              beta[ind], sigma[ind], temp_z, dX_z,
                              temp_max_z, alpha);
      ll += filter_segment_1d(idx_s, n_s, obs_y, dt_vec, hpe_vec,
                              beta[ind], sigma[ind], temp_z, dY_z,
                              temp_max_z, alpha);
    }
...
data {
  ...
  // Position based temp gradients gradient
  vector[N_total] dX;
  vector[N_total] dY;
...
transformed data {
  ...
  real temp_max_z = (temp_max - temp_mean) / temp_sd;
  
  vector[2 * N_total] combined_grad;
  combined_grad = append_row(dX, dY);
  
  real grad_sd = sd(combined_grad);
  vector[N_total] dX_z = dX / grad_sd;
  vector[N_total] dY_z = dY / grad_sd;
  ...
  
  {
    ...
    for (s in 1:N_segments) {
      ...
      log_lik[ind] += filter_segment_1d(idx_s, n_s, obs_x, dt, HPEm,
                                        beta[ind], sigma[ind],
                                        temp_z,  dX_z, temp_max_z, alpha);
                                        
      log_lik[ind] += filter_segment_1d(idx_s, n_s, obs_y, dt, HPEm,
                                        beta[ind], sigma[ind],
                                        temp_z,  dY_z, temp_max_z, alpha);
    }
  }
}
