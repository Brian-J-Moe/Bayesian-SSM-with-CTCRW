...
<span id="eq-gam">$$
\begin{aligned}
g(\text{ln}(HPEm)) \quad = & \quad s(\text{ln}(HPE)) + s(\text{ln1p}(RMSE)) + te(\text{ln}(R_{noise}), \text{ln}(R_{temp})) + \\ \\
                     & \quad te(\text{ln1p}(HPEm_{sys}), nR_{5m \epsilon}) + te(Day_{julian}, Hour) + \\ \\
                     & \quad ti(\text{ln}(HPE), Day_{julian}) + ti(\text{ln}(R_{noise}), Day_{julian}) + \\ \\
                     & \quad ti(\text{ln1p}(HPEm_{sys}), Day_{julian}) + ti(\text{ln1p}(RMSE), Hour) + \\ \\
                     & \quad ti(\text{ln}(R_{noise}), Hour) + ti(\text{ln1p}(RMSE), \text{ln}(R_{noise}))
\end{aligned}
 \qquad(1)$$</span>
